            ...
            steps {
                ...
                script {
                    ...
                    withCredentials([string(credentialsId: 'sonar', variable: 'sonarLogin')]) {
                        sh "${sonarqubeScannerHome}/bin/sonar-scanner -e -Dsonar.host.url=http://SonarQube:9000 -Dsonar.login=${sonarLogin} -Dsonar.projectName=mv-maven -Dsonar.projectVersion=${env.BUILD_NUMBER} -Dsonar.projectKey=GS -Dsonar.sources=src/main/java/cl/kibernumacademy/apirest -Dsonar.tests=src/test/java/cl/kibernumacademy/apirest -Dsonar.language=java -Dsonar.java.binaries=."
                    }
                }
            }
        }
